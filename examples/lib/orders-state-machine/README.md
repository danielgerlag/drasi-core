...
A self-driving order-lifecycle demo built on the Drasi **state machine** component.
It ties five components together and is verified end-to-end (orders animate all the
way from NEW to DELIVERED on a live dashboard).
...
| **State machine** (`order-state` reaction + `order-state-source` source) | Correlates the stage queries by `order_id` into the states `NEW → CONFIRMED → PAID → PICKED → SHIPPED → DELIVERED`, and exposes each order's live state as a source. |
...
```text
Postgres (orders, payments,   ──▶ orders-db ──▶ 6 stage queries ──┬──▶ state machine ──▶ order-state-source ──▶ dashboard queries ──▶ dashboard (:3000)
...
The state machine reaction persists each order's state to a durable
