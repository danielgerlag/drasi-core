...
//! Configuration types for the state machine reaction.
//!
...
//! allowed `previous` state, the entity transitions into that state.
...
#[schema(as = reaction::state_machine::Op)]
...
#[schema(as = reaction::state_machine::EnterCondition)]
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, ToSchema)]
#[schema(as = reaction::state_machine::StateDef)]
#[serde(rename_all = "camelCase")]
pub struct StateDef {
...
/// Configuration for the state machine reaction.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, ToSchema)]
#[schema(as = reaction::state_machine::StateMachineReactionConfig)]
#[serde(rename_all = "camelCase")]
pub struct StateMachineReactionConfig {
    /// Id under which the companion entity-state source is registered. Downstream
    /// queries subscribe to this id to read entity state.
...
impl Default for StateMachineReactionConfig {
    fn default() -> Self {
        Self {
...
impl StateMachineReactionConfig {
    ...
    pub fn validate(&self) -> anyhow::Result<()> {
        if self.source_id.trim().is_empty() {
            anyhow::bail!("state machine requires a non-empty source_id");
        }
...
mod tests {
    ...
    fn sample() -> StateMachineReactionConfig {
        StateMachineReactionConfig {
            source_id: "order-state-source".to_string(),
