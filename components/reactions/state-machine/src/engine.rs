...
//! performs no persistence or dispatch — those concerns live in the reaction and
//! source components.
...
use crate::config::{Op, StateMachineReactionConfig};
...
/// store; it is self-describing so the companion source can rebuild nodes during
/// bootstrap without access to the reaction config.
...
impl StateMachine {
    /// Build an engine from the reaction config, compiling all key templates.
    pub fn new(config: &StateMachineReactionConfig) -> Result<Self> {
...
mod tests {
    ...
    fn config() -> StateMachineReactionConfig {
