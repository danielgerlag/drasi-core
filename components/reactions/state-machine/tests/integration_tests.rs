...
//! End-to-end tests for the state machine reaction + companion source.
//!
//! These wire a real `DrasiLib` instance: an application source feeds order data
//! through three stage queries into the state machine reaction, which transitions
//! entities and pushes their state to the companion source. A downstream query
//! over the state source is observed via an application reaction.
...
use drasi_reaction_state_machine::{StateMachineBuilder, StateMachineSource};
...
async fn state_machine_progresses_and_exposes_state_source() {
    ...
    let store = Arc::new(RedbStateStoreProvider::new(tmp.path().join("state.redb")).unwrap());
    ...
    let (sm_reaction, sm_source) = StateMachineBuilder::new("order-state")
        .with_config(state_machine_config(sm_source_id))
        .build_pair()
        .unwrap();
...
async fn fresh_subscriber_bootstraps_from_persisted_state() {
    ...
    let sm_source = StateMachineSource::new(source_id).unwrap();
...
fn state_machine_config(
    source_id: &str,
) -> drasi_reaction_state_machine::StateMachineReactionConfig {
    drasi_reaction_state_machine::StateMachineReactionConfig {
